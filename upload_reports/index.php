    ...
    try {
      ...
      foreach (array_slice($rows, 1) as $row) {
        ...
        if ($isOrderHistory) {
          ...
          $memberComp = getCompany(strtoupper(substr($memberCode, 0, 2)) === 'SG' ? 'SGHQ' : 'MYHQ', $pdo);
          $memberStmt->execute([$memberComp['company_id'], $memberCode, getRowValue($row, $headerMap, ['membername']), getRowValue($row, $headerMap, ['mobileno'])]);
          $orderStmt->execute([$memberComp['company_id'], $batchId, $orderId, $datetime, $memberCode, getRowValue($row, $headerMap, ['membertype']), getRowValue($row, $headerMap, ['membername']), getRowValue($row, $headerMap, ['remark']), getRowValue($row, $headerMap, ['deliverymethod']), getRowValue($row, $headerMap, ['mobileno']), getRowValue($row, $headerMap, ['ordertype']), parseAmount(getRowValue($row, $headerMap, ['subtotal'])), parseAmount(getRowValue($row, $headerMap, ['shippingfee'])), parseAmount(getRowValue($row, $headerMap, ['voucherdiscount'])), parseAmount(getRowValue($row, $headerMap, ['discount'])), parseAmount(getRowValue($row, $headerMap, ['conveniencefee'])), parseAmount(getRowValue($row, $headerMap, ['ordertotal'])), parseAmount(getRowValue($row, $headerMap, ['gst'])), parseAmount(getRowValue($row, $headerMap, ['totalbv'])), parseAmount(getRowValue($row, $headerMap, ['totalpv'])), parseAmount(getRowValue($row, $headerMap, ['totaltp'])), getRowValue($row, $headerMap, ['paymentmode']), getRowValue($row, $headerMap, ['orderstatus']), getRowValue($row, $headerMap, ['deliverystatus']), getRowValue($row, $headerMap, ['paymentgateway']), getRowValue($row, $headerMap, ['paymentgatewayid']), $memberComp['currency_code'], $comp['invoice_prefix']]);
          $success++;
          continue;
        }
