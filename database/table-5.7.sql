...
CREATE TABLE `companies` (
    ...
    UNIQUE KEY `uq_companies_company_code`
        (`company_code`),

    KEY `idx_companies_is_active`
        (`is_active`)
) ENGINE=InnoDB
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE `order_items` (
    ...
    KEY `idx_order_items_order_id`
        (`order_id`),
    KEY `idx_order_items_item_code`
        (`item_code`),
    KEY `idx_order_items_order_item`
        (`order_id`, `item_code`),
    KEY `idx_order_items_product_type`
        (`product_type`),
    KEY `idx_order_items_email`
        (`email`),
...
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_unicode_ci
COMMENT='Line items from Tax Invoice Listing';
...
CREATE TABLE `sales_channels` (
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
        ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_sales_channels_channel_code`
        (`channel_code`),
    KEY `idx_sales_channels_is_active`
        (`is_active`)
) ENGINE=InnoDB
DEFAULT CHARSET=utf8mb4
COLLATE=utf8mb4_unicode_ci
...
-- ============================================================
INSERT IGNORE INTO `sales_channels`
(
    `channel_code`,
    `channel_name`
)
...
CREATE TABLE `manual_sales` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `company_id` BIGINT UNSIGNED NOT NULL
        COMMENT 'FK to companies.id - for currency detection',
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
        ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_manual_sales_company_channel_date`
    (
        `company_id`,
        `sales_channel_id`,
        `sales_date`,
    ...
    KEY `idx_manual_sales_sales_date`
        (`sales_date`),
    KEY `idx_manual_sales_brand`
        (`brand`),
    KEY `idx_manual_sales_company_id`
        (`company_id`),
...
CREATE TABLE `sales_target` (
    ...
    `target_date` DATE NOT NULL
        COMMENT 'One row = one daily overall target',
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
        ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_sales_target_date`
        (`target_date`),
    KEY `idx_sales_target_created_by`
        (`created_by`),
